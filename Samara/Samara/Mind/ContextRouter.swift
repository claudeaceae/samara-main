...
final class ContextRouter {
    ...
    struct ContextNeeds {
        ...
        var estimatedTokens: Int {
            var tokens = 3000  // Core context always loaded

            if needsCapabilities { tokens += 1000 }
            if needsDecisions { tokens += 800 }
            if needsLearnings { tokens += 800 }
            if needsObservations { tokens += 800 }
            if needsLocationContext { tokens += 300 }
            if needsCalendarContext { tokens += 500 }
            if needsTodayEpisode { tokens += 1000 }

            tokens += needsPersonProfiles.count * 500
            tokens += searchQueries.count * 200
    ...
    init(claudePath: String = "/usr/local/bin/claude", timeout: TimeInterval = 5.0, enabled: Bool = true) {
    ...
    func analyze(_ messages: [Message]) -> ContextNeeds {
        ...
        let combinedText = messages.map { $0.text }.joined(separator: "\n")
    ...
    private func parseClassification(_ output: String) -> Classification? {
        ...
        var jsonString = output.trimmingCharacters(in: .whitespacesAndNewlines)

        // Handle potential wrapper from claude CLI
        ...
        do {
            ...
            log("Failed to parse classification: \(error)", level: .warn, component: "ContextRouter")
            return nil
        }
    }
